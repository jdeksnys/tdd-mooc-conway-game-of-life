...
import {GenerateFullRleReponse} from "../src/app.mjs"
...
describe("Combine all logic", () => {
  ...
  test("path+number -> full rle result (rle+comments+size)", () => {
    let path = "test/glider.rle.txt";
    let no = 3;
    expect(GenerateFullRleReponse(path, no)).to.equal(
...
o2b$b2o$2ob!`);
  ...
  test("throw file not found error", () => {
    let path = "test/random_blablabla.rle.txt";
    let no = 3;
    let error;
    try{
      let test = GenerateFullRleReponse(path, no);
    } catch(e){
      error = e;
    }
